...
class Login extends CI_Controller
{
    ...
    public function validate_login($from = "")
    {
        ...
        if ($query->num_rows() > 0) {
            ...
            if ($row->role_id == 1) {
                ...
                redirect(site_url('home/new_certificate'), 'refresh');
            }
        } else {
    ...
    public function register()
    {
        ...
        //this hidding function is get system username 
        ...
        $hash_email1 = $data['email'] ;
        $hash_email = do_hash($hash_email1);
        ...
        if ($validity === 'unverified_user' || $validity == true) {
            ...
            if (get_settings('student_email_verification') == 'enable') {
                ...
                if ($validity === 'unverified_user') {
                   ...
                   // $hash_email = $data['email'] ;
                     // its worked
                $dir_temp = "working_directory/"; 
                $oldmask = umask(0);
                mkdir($dir_temp.$hash_email."/certificate", 0777,true);
                mkdir($dir_temp.$hash_email."/backgroundimg", 0777,true);
                mkdir($dir_temp.$hash_email."/logo", 0777,true);
                mkdir($dir_temp.$hash_email."/keyfiles", 0777,true); 
                umask($oldmask);
                // its one way of folder creation code for strategy login the user then create the four directory files
                ...
                redirect(site_url('home/verification_code'), 'refresh');
                 ...
                 // its worked this code for strategynaveen 
                 $dir_temp = "working_directory/"; 
                 $oldmask = umask(0);
                 mkdir($dir_temp.$hash_email."/certificate", 0777,true);
                 mkdir($dir_temp.$hash_email."/backgroundimg", 0777,true);
