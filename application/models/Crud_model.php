...
// strategy code json download
public function download_json_detail($userid,$roleid){
    $this->db->select('email,first_name,last_name');
    $this->db->where('id',$userid);
    $this->db->where('role_id',$roleid);
    $result_json = $this->db->get('users');
    $res_count = $result_json->result();
...
// strategy code download pdf
public function download_pdf_data($userid,$rollid){
    $this->db->select('email,first_name,last_name');
    $this->db->where('id',$userid);
    $this->db->where('role_id',$roleid);
    $result_pdf = $this->db->get('users');
    $res_count_pdf = $result_pdf->result();

    foreach($res_count_pdf as $row){
        $data['email'] = $row->email;
        $data['first_name'] = $row->first_name;
        $data['last_name'] = $row->last_name;
...
// startegy code admin logo images
/*
common logo images
*/
public function logoimg(){
    $this->db->select("logoname");
    $this->db->from("admin_logoimg");
    $result = $this->db->get();
    $res = $result->result();
    return $res;
}
...
public function user_background_img($data){
    ...
    $res = $this->db->insert('user_backgroundimg',$ins);
...
public function user_logo_add($data){
    $userid = $data['userid'];
    $email = $data['email'];
    $imgname = $data['imgname'];
    $imgt = $data['imgtype'];
    ...
    $res_logo = $this->db->insert('user_backgroundimg',$lins);
    if ($res_logo == true) {
        return true;
    }
    else{
...
// load user background images
public function user_bgimg_select($userid,$email){
    $this->db->select("imgname");
    $this->db->from("user_backgroundimg");
    $this->db->where('userid',$userid);
    $this->db->where('imgtype','b');
    $res = $this->db->get();
    $result = $res->result();
...
// load user logo images
public function user_limg_select($userid,$email){
    $this->db->select('imgname');
    $this->db->from('user_backgroundimg');
    $this->db->where('userid',$userid);
    $this->db->where('imgtype','l');
    $result = $this->db->get();
    $res = $result->result();
    return $res;
}
